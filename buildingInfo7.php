...
<?php
	...
	if($stmt->prepare("select * from building where bid = 7") or die(mysqli_error($db))) {
...
?>
	...
	if($stmt->prepare("select * from buildingInfo where yearOfConstruction = 1898") or die(mysqli_error($db))) {
		$stmt->execute();
...
$query = "SELECT bID, spaceID, spaceType from studySpaceType NATURAL JOIN studySpace WHERE bid = 7 GROUP By spaceType";
...
$query = "SELECT * from foodService where bID = 7";
$result = $db ->query($query);
...
$query = "SELECT * from meetingRoom where bID = 7";
$result = $db ->query($query);
