...
<?php
	...
	if($stmt->prepare("select * from building where bid = 8") or die(mysqli_error($db))) {
...
?>
	...
	if($stmt->prepare("select * from buildingInfo where yearOfConstruction = 1938") or die(mysqli_error($db))) {
		$stmt->execute();
...
$query = "SELECT bID, spaceID, spaceType from studySpaceType NATURAL JOIN studySpace WHERE bid = 8 GROUP By spaceType";
$result = $db ->query($query);

echo '<span style="color:#0511ff;text-decoration: underline; font-family:courier; text-align:;"> The available study spaces at this building are: </span>'; 
...
$query = "SELECT * from foodService where bID = 8";
$result = $db ->query($query);

echo '<span style="color:#0511ff;text-decoration: underline; font-family:courier; text-align:;"> You can find the following restaurants at this bulding: </span>'; 
...
$query = "SELECT * from meetingRoom where bID = 8";
$result = $db ->query($query);
