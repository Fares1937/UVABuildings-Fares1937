...
<?php
	...
	if($stmt->prepare("select * from building where bid = 9") or die(mysqli_error($db))) {
...
?>
	...
	if($stmt->prepare("select * from buildingInfo where yearOfConstruction = 1876") or die(mysqli_error($db))) {
		$stmt->execute();
...
$query = "SELECT bID, spaceID, spaceType from studySpaceType NATURAL JOIN studySpace WHERE bid = 9 GROUP By spaceType";
...
$query = "SELECT * from foodService where bID = 9";
$result = $db ->query($query);
...
$query = "SELECT * from meetingRoom where bID = 9";
$result = $db ->query($query);
