...
<?php
	...
	if($stmt->prepare("select * from building where bid = 3") or die(mysqli_error($db))) {
...
?>
	...
	if($stmt->prepare("select * from buildingInfo where yearOfConstruction = 1932") or die(mysqli_error($db))) {
		$stmt->execute();
...
<?php
	...
	if($stmt->prepare("select * from school where sID = 8") or die(mysqli_error($db))) {
		$stmt->execute();
...
$query = "SELECT bID, spaceID, spaceType from studySpaceType NATURAL JOIN studySpace WHERE bid = 3 GROUP By spaceType";
...
$query = "SELECT * from foodService where bID = 3";
$result = $db ->query($query);
...
while ($row = $result->fetch_assoc()) {
	?> 
			<span style="color:#ff7e0c;font-family:courier; ext-align:;"><p><?php echo "&#8226"; echo $row['fsName'];?></p></span>
...
$query = "SELECT * from meetingRoom where bID = 3";
$result = $db ->query($query);
