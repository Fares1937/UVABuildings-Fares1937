...
<?php
	...
	if($stmt->prepare("select * from building where bid = 6") or die(mysqli_error($db))) {
...
?>
	...
	if($stmt->prepare("select * from buildingInfo where yearOfConstruction = 1950") or die(mysqli_error($db))) {
		$stmt->execute();
...
$query = "SELECT bID, spaceID, spaceType from studySpaceType NATURAL JOIN studySpace WHERE bid = 6 GROUP By spaceType";
$result = $db ->query($query);

echo '<span style="color:#0511ff;text-decoration: underline; font-family:courier; text-align:span;"> The available study spaces at this building are: </span>'; 
...
$query = "SELECT * from foodService where bID = 6";
$result = $db ->query($query);
...
while ($row = $result->fetch_assoc()) {
	?> 
			<span style="color:#ff7e0c;text-align:;"> <p><?php echo "&#8226"; echo $row['fsName'];?></p></span>
...
$query = "SELECT * from meetingRoom where bID = 6";
$result = $db ->query($query);
