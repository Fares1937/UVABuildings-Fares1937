...
<html>
   <?php
	...
	<form method="post">
   		<input type="submit" name="save" class ="bbtn btn-primary btn-sm" value="Save Building"/>
	</form>
...
<form method="post">
	       ...
	       <button  type="submit" name="rate" class ="bbtn btn-warning btn-sm">Rate!</button>
</form>
<?php
			$dbconn = DbUtil::loginConnection();
			$con = new mysqli(DbUtil::$host, DbUtil::$loginUser, DbUtil::$loginPass, DbUtil::$schema);
...
<?php
	...
	if($stmt->prepare("select * from building where bid = 1") or die(mysqli_error($db))) {
...
<?php
	...
	if($stmt->prepare("select * from schoolInfo where numStudents = 484") or die(mysqli_error($db))) {
		$stmt->execute();
...
<?php
	...
	if($stmt->prepare("select * from buildingInfo where yearOfConstruction = 1826") or die(mysqli_error($db))) {
		$stmt->execute();
...
$query = "SELECT bID, spaceID, spaceType from studySpaceType NATURAL JOIN studySpace WHERE bid = 1 GROUP By spaceType";
...
$query = "SELECT * from foodService where bID = 1";
$result = $db ->query($query);
...
while ($row = $result->fetch_assoc()) {
	?> 
			<span style="color:#ff7e0c;font-family:courier; font-family:courier; text-align:;"><p><?php echo "&#8226"; echo $row['fsName'];?></p></span>
...
$query = "SELECT * from meetingRoom where bID = 1";
$result = $db ->query($query);
