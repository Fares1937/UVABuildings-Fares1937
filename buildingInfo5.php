...
<?php
	...
	if($stmt->prepare("select * from building where bid = 5") or die(mysqli_error($db))) {
...
?>
	...
	if($stmt->prepare("select * from buildingInfo where yearOfConstruction = 1962") or die(mysqli_error($db))) {
		$stmt->execute();
...
<?php
	...
	if($stmt->prepare("select * from school where sID = 5") or die(mysqli_error($db))) {
		$stmt->execute();
		...
		?>
			...
			<span style="color:#ff7e0c;font-family:courier; text-align:center;"><p><?php echo 'This building belongs to'; echo '&nbsp'; echo $sName;?></p></span>
		<br>
		<?php
...
$query = "SELECT bID, spaceID, spaceType from studySpaceType NATURAL JOIN studySpace WHERE bid = 5 GROUP By spaceType";
...
$query = "SELECT * from foodService where bID = 5";
$result = $db ->query($query);
...
$query = "SELECT * from meetingRoom where bID = 5";
$result = $db ->query($query);
