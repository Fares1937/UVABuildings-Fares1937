...
<?php
	...
	if($stmt->prepare("select * from building where bid = 2") or die(mysqli_error($db))) {
...
?>
	...
	if($stmt->prepare("select * from buildingInfo where yearOfConstruction = 1958") or die(mysqli_error($db))) {
		$stmt->execute();
...
$query = "SELECT bID, spaceID, spaceType from studySpaceType NATURAL JOIN studySpace WHERE bid = 2 GROUP By spaceType";
...
while ($row = $result->fetch_assoc()) {
	?> 
			<span style="color:#ff7e0c;font-family:courier; text-align:;"> <p><?php echo "&#8226"; echo $row['spaceType'];?></p>
 </span>
	<?php
...
$query = "SELECT * from foodService where bID = 2";
$result = $db ->query($query);

echo '<span style="color:#0511ff; text-decoration: underline; font-family:courier; text-align:center;"> You can find the following restaurants at this building: </span>'; 
				
while ($row = $result->fetch_assoc()) {

		
	?> 
			<span style="color:#ff7e0c;font-family:courier; text-align:;"> <p><?php echo "&#8226"; echo $row['fsName'];?></p>
			</span>
...
<?php
	...
	if($stmt->prepare("select * from foodService where bID = 10") or die(mysqli_error($db))) {
		...
		?>
			...
			<span style="color:#0511ff;font-family:courier; text-align:;"> <p><?php echo 'This building has'; echo '&nbsp'; echo $microwave; echo '&nbsp';  echo 'microwave';?></p> </span>
		<br>
		<?php
...
$query = "SELECT * from meetingRoom where bID = 2";
$result = $db ->query($query);
...
while ($row = $result->fetch_assoc()) {
	?> 
			<span style="color:#ff7e0c;font-family:courier; text-align:;"> <p><?php echo $row['roomNumber'];?></p>
</span>
	<?php
